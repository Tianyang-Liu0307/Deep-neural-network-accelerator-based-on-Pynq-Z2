...
{
#pragma HLS ARRAY_PARTITION variable=IFM block factor=8 dim=1
...
{
#pragma HLS ARRAY_PARTITION variable=OFM block factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=IFM block factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=WBUF3x3 complete dim=0
...
#pragma HLS ARRAY_PARTITION variable=line_buffer complete dim=2
    ...
    for (int row_in = 0; row_in < 83; row_in++)
    {
        ...
        #pragma HLS LOOP_FLATTEN
        #pragma HLS PIPELINE II=1
            DWCONV3X3_label0:for (int c = 0; c < 32; c++)
            {
            //#pragma HLS UNROLL
...
#pragma HLS ARRAY_PARTITION variable=W1x1 dim=2 complete
	...
	for(int ci=0; ci<32; ci+=16)
    {
		...
		for(int h=1; h<42; h++)
        {
            ...
            {
#pragma HLS PIPELINE II=1
				PWCONV1X1_label0:for(int co=0; co<32; co++)
                {
//#pragma HLS UNROLL
...
{
#pragma HLS INLINE off
#pragma HLS ARRAY_PARTITION variable=BBUF block factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=MBUF block factor=2 dim=1
    for (int h = 1; h < 42; h++)
    {
        for (int w = 1; w < 82; w++)
        {
